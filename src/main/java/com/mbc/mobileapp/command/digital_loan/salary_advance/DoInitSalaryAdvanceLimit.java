    ...
    public boolean execute(Context cntxt) throws Exception {
        ...
        try {
            ...
            if (custT24 != null) {
                ...
                String occupation = Optional.ofNullable(custT24)
                        .map(CustomerInfoT24::getEmploymentStatusInfo)
                        .orElse(Collections.emptyList())
                        .stream()
                        .filter(Objects::nonNull)
                        .filter(e -> e.getOccupation() != null)
                        .sorted(Comparator.comparing(e -> e.getJobTitle() == null ? 1 : 0))
                        .map(CustomerOccupation::getOccupation)
                        .findFirst()
                        .orElse(null);

                tempRecord.setOccupation(occupation);
